...
function requireUser(store, req, res) {
  ...
  if (user) return user;
  const next = encodeURIComponent(req.originalUrl || '/console');
...
function mountAccountRoutes(app, store) {
  app.get(['/login', '/signin'], (req, res) => {
    if (cookieParserUser(store, req)) return res.redirect(302, '/console');
  ...
  app.get('/console/billing', (req, res) => {
    const user = requireUser(store, req, res);
    if (!user) return;
    res.type('html').send(billingPage({
      user,
      balanceCents: store.balance(user.id),
      error: req.query.error,
      notice: req.query.notice,
  ...
  app.post(['/console/start', '/account/start'], async (req, res) => {
    const user = requireUser(store, req, res);
    if (!user) return;
    const sku = String((req.body && req.body.sku) || req.query.sku || '').trim();
    if (!sku) return res.redirect(303, '/console?error=' + encodeURIComponent('Pick a GPU.'));
    try {
      const offers = await fetchOffers();
      const offer = offers.find((o) => o.sku === sku || o.name === sku) || {
        sku, name: sku, usdPerHour: Number(req.body.usdPerHour || 0), region: req.body.region || '', tier: 'on-demand',
      };
      ...
      const machine = store.createMachine(user.id, {
        sku: offer.sku || sku, name: offer.name || sku, usdPerHour: offer.usdPerHour || 0, region: offer.region || '', status: 'starting',
      });
      ...
      try {
        ...
        if (!upstream.ok) {
          ...
          store.updateMachine(machine.id, { status: (data.pod && data.pod.status) || 'running', remoteId: data.pod && data.pod.id, message: 'Live on Aorila capacity.' });
        }
      } catch {
        store.updateMachine(machine.id, { status: 'queued', message: 'Recorded. Control plane will retry the drop.' });
      }
      res.redirect(303, '/console?notice=' + encodeURIComponent('Machine created.'));
    } catch (err) {
      res.redirect(303, '/console?error=' + encodeURIComponent(err.message || 'Could not start.'));
    }
  });
  ...
  app.post('/console/stop/:id', async (req, res) => {
    ...
    if (machine.remoteId) {
      try { await fetch(COMPUTE + '/compute/v1/pods/' + encodeURIComponent(machine.remoteId) + '/stop', { method: 'POST' }); } catch {}
    }
    store.updateMachine(machine.id, { status: 'stopped', message: 'Stopped from console.' });
    res.redirect(303, '/console?notice=' + encodeURIComponent('Machine stopped.'));
  });

  app.get('/commercial/console', (req, res) => {
    const user = requireUser(store, req, res);
    if (!user) return;
    res.type('html').send(commercialConsole({
      user, bookings: store.bookingsFor(user.id), error: req.query.error, notice: req.query.notice,
    }));
  });
