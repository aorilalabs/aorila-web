...
function sendConsole(res, store, user, req, section) {
  res.type('html').send(
    consolePage({
      ...
      clusters: store.clustersFor(user.id),
      deployments: store.deploymentsFor(user.id),
...
// Provision a cluster: N pods from ONE live catalog offer. Never invents an
// offer or a price — if the offer is not in the live catalog, refuse.
// One hour of credit per node is held up front.
async function provisionCluster(store, user, { offer, offerId, name, nodes, image, diskGb }) {
  let offers = [];
  try {
  ...
  const live =
    (offerId && offers.find((o) => o.offerId === offerId || o.sku === offerId)) ||
    (offer && offers.find((o) => o.offerId === offer || o.sku === offer || o.name === offer));
  if (!live || !Number(live.usdPerHour)) {
    const err = new Error('That GPU is not in the live catalog right now. Pick a live offer.');
    err.status = 400;
    throw err;
  }
  const n = Math.max(1, Math.min(64, Number(nodes || 2)));
  const hourCents = Math.max(1, Math.round(Number(live.usdPerHour) * 100));
  const holdCents = hourCents * n;
  if (store.balance(user.id) < holdCents) {
    const err = new Error(`Add credits before launching: ${n} nodes need $${(holdCents / 100).toFixed(2)} for the first hour.`);
    err.status = 402;
    throw err;
  }
  const clusterName = String(name || 'aorila-cluster').trim() || 'aorila-cluster';
  store.addCredits(user.id, { cents: -holdCents, note: `hold 1h cluster ${clusterName} x${n}` });
  const machineIds = [];
  for (let i = 0; i < n; i++) {
    const machine = store.createMachine(user.id, {
      sku: live.sku || live.name || '',
      name: `${clusterName}-${i + 1}`,
      usdPerHour: live.usdPerHour,
      region: live.region || '',
      image: image || 'aorila/pytorch',
      diskGb: diskGb || 20,
      jupyterOn: true,
      status: 'starting',
    });
    ...
    try {
      const upstream = await fetch(COMPUTE + '/compute/v1/pods', {
        ...
        body: JSON.stringify({
          sku: live.sku || live.name || '',
          offerId: live.offerId || undefined,
          name: machine.name,
  ...
  const cluster = store.createCluster(user.id, {
    name: clusterName,
  ...
  for (const id of machineIds) store.updateMachine(id, { clusterId: cluster.id });
  return cluster;
}
...
function mountAccountRoutes(app, store) {
  ...
  app.get('/console/clusters/new', async (req, res) => {
    const user = requireUser(store, req, res);
    if (!user) return;
    res.type('html').send(
      newClusterPage({
  ...
  app.post('/console/clusters', async (req, res) => {
    const user = requireUser(store, req, res);
    if (!user) return;
    const body = req.body || {};
    try {
      const cluster = await provisionCluster(store, user, {
        offer: body.offer,
        offerId: body.offer,
        name: body.name,
        nodes: body.nodes,
      ...
      res.redirect(303, '/console/clusters?notice=' + encodeURIComponent(`Cluster ${cluster.name} launched with ${cluster.nodes} nodes.`));
    } catch (err) {
      res.redirect(303, '/console/clusters/new?error=' + encodeURIComponent(err.message || 'Could not launch.'));
    }
  });

  app.get('/console/deployments/new', async (req, res) => {
    const user = requireUser(store, req, res);
    if (!user) return;
    res.type('html').send(
      newDeploymentPage({
  ...
  app.post('/console/deployments', (req, res) => {
    const user = requireUser(store, req, res);
    if (!user) return;
    const body = req.body || {};
    const offer = String(body.offer || '').trim();
    if (!offer) return res.redirect(303, '/console/deployments/new?error=' + encodeURIComponent('Pick a live catalog offer.'));
    try {
      const dep = store.createDeployment(user.id, {
        name: body.name,
        offerId: offer,
        sku: offer,
        nodes: body.nodes,
      ...
      res.redirect(303, '/console/deployments?notice=' + encodeURIComponent(`Deployment ${dep.name} saved.`));
    } catch (err) {
      res.redirect(303, '/console/deployments/new?error=' + encodeURIComponent(err.message || 'Could not save.'));
    }
  });

  app.post('/console/deployments/:id/launch', async (req, res) => {
    const user = requireUser(store, req, res);
    if (!user) return;
    const dep = store.deploymentById(user.id, req.params.id);
    if (!dep) return res.redirect(303, '/console/deployments?error=' + encodeURIComponent('Deployment not found.'));
    try {
      // The spec stores the offer reference, not a price: the launch resolves
      // the live catalog price at launch time, like a fresh cluster.
      const cluster = await provisionCluster(store, user, {
        offer: dep.offerId || dep.sku,
        offerId: dep.offerId || dep.sku,
        name: dep.name,
        nodes: dep.nodes,
        image: dep.image,
        diskGb: dep.diskGb,
      });
      res.redirect(303, '/console/clusters?notice=' + encodeURIComponent(`Deployment ${dep.name} launched as cluster ${cluster.name}.`));
    } catch (err) {
      res.redirect(303, '/console/deployments?error=' + encodeURIComponent(err.message || 'Could not launch.'));
    }
  });

  app.post('/console/deployments/:id/delete', (req, res) => {
    const user = requireUser(store, req, res);
    if (!user) return;
    if (!store.deleteDeployment(user.id, req.params.id)) {
      return res.redirect(303, '/console/deployments?error=' + encodeURIComponent('Deployment not found.'));
    }
    res.redirect(303, '/console/deployments?notice=' + encodeURIComponent('Deployment deleted.'));
  });
