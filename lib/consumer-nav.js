/** Shared consumer header: Ally / Labs visible; APIs under Product; marketing menu in right sidebar. */
...
function chip(href, label, on, attrs) {
  const cls = on ? 'nav-commercial' : 'nav-link';
  const current = on ? ' aria-current="page"' : '';
  return `<a class="${cls}" href="${esc(href)}"${current}${attrs || ''}>${esc(label)}</a>`;
}
...
function consumerNav(opts = {}) {
  ...
  const onLabs = Boolean(opts.labsCurrent);
  const onAlly = Boolean(opts.allyCurrent);
  const onAorila = !onCommercial && !onLabs && !onAlly;

  const product = dropdown('Product', [
    {
      href: 'https://api.aorila.com',
      title: 'APIs',
      desc: 'Commercial API access for builders and companies',
    },
  ...
  return `<header class="nav">
    ${chip(wordmarkHref, 'Aorila', onAorila, aorilaAttrs)}
    ...
    <nav class="nav-links nav-utility" aria-label="Aorila products">
      ...
      ${chip('https://aorilalabs.com', 'Labs', onLabs, ' data-local-site="labs"')}
    </nav>
