...
function consumerNav(opts = {}) {
  ...
  const user = opts.accountUser;

  const product = dropdown('Product', [
    { href: 'https://api.aorila.com', title: 'APIs', desc: 'AI API — not GPU rental' },
    ...
    { href: '/deployments', title: 'Deployments / Models', desc: 'One control plane' },
  ]);
  const productNav = apiCurrent
    ? product.replace('href="https://api.aorila.com"', `href="https://api.aorila.com"${apiCurrent}`)
    : product;
  ...
  const accountLinks = user
    ? `<a class="nav-link" href="/console">Console</a><a class="nav-link" href="/console/billing">Billing</a><form method="post" action="/logout" class="nav-logout"><button class="nav-link" type="submit">Sign out</button></form>`
    : `<a class="nav-link" href="/login">Sign in</a><a class="nav-cta" href="/signup">Create account</a>`;
