...
function consoleWrap({ title, user, balanceCents, section, main }) {
  const first = String(user && user.name ? user.name : 'there').split(' ')[0];
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
...
function machineList(machines) {
  ...
  return machines
    .map(
      (m) => `<div class="console-pod"><div><b>${esc(m.name || m.sku)}</b><div class="console-meta"><span>$${Number(m.usdPerHour || 0).toFixed(2)}/hr</span><span>${esc(m.sku || '')}</span><span>${esc(m.status)}</span></div></div>${m.status === 'stopped' || m.status === 'failed' ? '' : `<form method="post" action="/console/stop/${esc(m.id)}"><button class="cta ghost" type="submit">Stop</button></form>`}</div>`
    )
    .join('');
}
...
function bookingRows(bookings) {
  ...
  return (
    ...
    bookings
      .map(
        (b) =>
          `<tr><td>${esc(b.name || b.sku)}</td><td>${esc(b.kind)}</td><td>${esc(b.window || '—')}</td><td>${esc(b.status)}</td></tr>`
      )
      .join('') +
    '</tbody></table></div>'
  );
...
function consolePage({ user, balanceCents, machines, bookings, error, notice, section }) {
  ...
  if (sec === 'billing') {
    ...
    main = `${flash}<h1>Ahoy, ${esc(first)}!</h1>${actions}${tabs}<div class="console-card"><h2>Recent Pods</h2>${machineList(machines)}</div>${bookings && bookings.length ? `<div class="console-card"><h2>Commercial</h2>${bookingRows(bookings)}</div>` : ''}`;
  }
