...
function clusterList(clusters) {
  if (!clusters.length) return '<p class="console-note">No clusters yet. Launch one from a live catalog offer.</p>';
  return clusters
    .map(
      (c) =>
        `<div class="console-pod"><div><b>${esc(c.name)}</b><div class="console-meta"><span>${esc(String(c.nodes))} nodes</span><span>${esc(c.sku || '')}</span><span>${esc(c.status)}</span></div></div><div class="console-actions"><a class="cta ghost" href="/console/pods">Nodes</a></div></div>`
    )
    .join('');
}

function deploymentList(deployments) {
  if (!deployments.length) return '<p class="console-note">No deployments yet. Save a launch spec to re-launch the same cluster any time.</p>';
  return deployments
    .map(
      (d) =>
        `<div class="console-pod"><div><b>${esc(d.name)}</b><div class="console-meta"><span>${esc(String(d.nodes))} nodes</span><span>${esc(d.sku || '')}</span><span>${esc(d.image || '')}</span></div></div><div class="console-actions"><form method="post" action="/console/deployments/${esc(d.id)}/launch"><button class="cta primary" type="submit">Launch</button></form><form method="post" action="/console/deployments/${esc(d.id)}/delete"><button class="cta ghost" type="submit">Delete</button></form></div></div>`
    )
    .join('');
}

// Server-rendered offer picker: radios over the LIVE catalog only. Never a
// typed-in price — the route re-validates the offer server-side at launch.
function offerPicker(offers, fieldName) {
  if (!offers.length) {
    return '<p class="console-note">No hosts online right now — the catalog opens as hosts list their GPUs. <a href="/providers">Become a host</a>.</p>';
  }
  return (
    ...
    offers
      ...
      .map(
        (o, i) =>
          `<label class="check"><input type="radio" name="${esc(fieldName)}" value="${esc(o.offerId || o.sku || '')}"${i === 0 ? ' checked' : ''} /> <b>${esc(o.name || o.sku || 'GPU')}</b> <span class="console-meta">$${Number(o.usdPerHour || 0).toFixed(3)}/hr · ${esc(o.region || 'global')}</span></label>`
      )
      .join('') +
    '</div><p class="console-note">Price is the live catalog price at launch time. One hour per node is held from your balance first.</p>'
  );
}
...
function newChooserPage({ user, balanceCents, error, notice }) {
  const main = `${flashBlock(error, notice)}<h1>New</h1><p class="console-note">Pick what to start.</p>
    <div class="chooser-grid">
      ...
      <a class="chooser" href="/console/clusters/new"><b>Cluster</b><span>Multi-node jobs from the live catalog.</span></a>
    </div>`;
...
function newClusterPage({ user, balanceCents, offers, error, notice }) {
  const main = `${flashBlock(error, notice)}<h1>Launch a cluster</h1>
    <p class="console-note">N pods from one live catalog offer. Balance ${money(balanceCents)} — one hour per node is held before launch.</p>
    <form class="waitlist" method="post" action="/console/clusters">
      <label>Name<input name="name" value="aorila-cluster" required /></label>
      <label>Nodes<input name="nodes" type="number" min="1" max="64" value="2" required /></label>
      <label>Image<select name="image"><option value="aorila/pytorch">Aorila PyTorch</option><option value="aorila/jupyter">Aorila Jupyter</option></select></label>
      ${offerPicker(offers || [], 'offer')}
      <button class="cta primary" type="submit">Launch cluster</button>
    </form>`;
  return consoleWrap({ title: 'New cluster', user, balanceCents, section: 'clusters', main });
}

function newDeploymentPage({ user, balanceCents, offers, error, notice }) {
  const main = `${flashBlock(error, notice)}<h1>New deployment</h1>
    <p class="console-note">Save a launch spec. Launching it provisions a cluster from the live catalog at that moment's price — never a stored price.</p>
    <form class="waitlist" method="post" action="/console/deployments">
      <label>Name<input name="name" value="aorila-deployment" required /></label>
      <label>Nodes<input name="nodes" type="number" min="1" max="64" value="2" required /></label>
      <label>Image<select name="image"><option value="aorila/pytorch">Aorila PyTorch</option><option value="aorila/jupyter">Aorila Jupyter</option></select></label>
      <label>Disk (GB)<input name="diskGb" type="number" min="10" value="20" /></label>
      ${offerPicker(offers || [], 'offer')}
      <button class="cta primary" type="submit">Save deployment</button>
    </form>`;
  return consoleWrap({ title: 'New deployment', user, balanceCents, section: 'deployments', main });
}

function consolePage({ user, balanceCents, machines, bookings, endpoints, volumes, clusters, deployments, credits, error, notice, section }) {
  ...
  } else if (sec === 'clusters') {
    main = `${flash}<h1>Clusters</h1><div class="console-actions"><a class="cta primary" href="/console/clusters/new">+ New cluster</a></div><div class="console-card"><h2>Clusters</h2>${clusterList(clusters || [])}</div>`;
  } else if (sec === 'deployments') {
    main = `${flash}<h1>Deployments</h1><div class="console-actions"><a class="cta primary" href="/console/deployments/new">+ New deployment</a></div><div class="console-card"><h2>Saved launch specs</h2>${deploymentList(deployments || [])}</div><p class="console-note">Launching a deployment provisions a fresh cluster from the live catalog at the current price.</p>`;
  } else if (sec === 'pods') {
    main = `${flash}<h1>Pods</h1>${actions}<div class="console-card"><h2>Recent Pods</h2>${machineList(machines)}</div>${liveBlock()}`;
  } else {
    main = `${flash}<h1>Welcome back, ${esc(first)}.</h1>${actions}${tabs}${agentCard()}<div class="console-card"><h2>Recent Pods</h2>${machineList(machines)}</div>${bookings && bookings.length ? `<div class="console-card"><h2>Capacity</h2>${bookingRows(bookings)}</div>` : ''}`;
  }
