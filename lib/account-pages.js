...
function consolePage({ user, balanceCents, machines, bookings, endpoints, volumes, credits, error, notice, section }) {
  ...
  if (sec === 'billing') {
    ...
    main = `${flash}<h1>Ahoy, ${esc(first)}!</h1>${actions}${tabs}${agentCard()}<div class="console-card"><h2>Recent Pods</h2>${machineList(machines)}</div>${bookings && bookings.length ? `<div class="console-card"><h2>Enterprise</h2>${bookingRows(bookings)}</div>` : ''}`;
  }
