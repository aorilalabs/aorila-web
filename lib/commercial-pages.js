...
function rail(active) {
  ...
  return `<nav class="chips" aria-label="Commercial" style="margin-bottom:26px">` + items.map(([slug, label]) => {
    const href = slug === 'console' ? '/commercial/console' : slug ? `/commercial/${slug}` : '/commercial';
    const cur = slug === active ? ' acid' : '';
    return `<a class="chip${cur}" style="text-decoration:none;color:#10100f;padding:8px 14px" href="${href}"${cur ? ' aria-current="page"' : ''}>${esc(label)}</a>`;
