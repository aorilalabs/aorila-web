...
const PAGES = {
  '': { title: 'Enterprise', heading: 'Aorila for organizations', lede: 'Reserved machines, planned VMs, and volume terms. On-demand pods stay on the public catalog.' },
...
function rail(active) {
  ...
  return `<nav class="chips" aria-label="Enterprise" style="margin-bottom:26px">` + items.map(([slug, label]) => {
    const href = slug === 'console' ? '/commercial/console' : slug ? `/commercial/${slug}` : '/commercial';
    const cur = slug === active ? ' acid' : '';
    const ink = cur ? '#ffffff' : '#10100f';
