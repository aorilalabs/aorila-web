...
const PAGES = {
  'ai-api': { title: 'AI API', badge: 'Product', heading: 'AI API.', lede: 'Atraly and Aorila models on atraly.com — one API for chat, voice, and agents.' },
  compute: { title: 'Compute', badge: 'Product', heading: 'One product for every GPU workload.', lede: 'On-demand pods, serverless endpoints, multi-node clusters, and hub templates — one control plane, one Aorila bill.' },
...
function renderMarketingPage(slug, { query, user } = {}) {
  ...
  if (slug === 'contact') {
    ...
    below =
      '<section class="section"><div class="wrap"><div class="section-head"><span class="snum">INCLUDED</span><div><h2>Everything in one product</h2></div></div><div class="cards">' +
      [
        { name: 'Pods', note: 'On-demand GPUs from live pools', href: '/console/pods' },
        { name: 'Serverless', note: 'Serverless GPU endpoints', href: '/console/serverless' },
        { name: 'Clusters', note: 'Multi-node GPU clusters', href: '/console/clusters' },
        { name: 'Hub', note: 'Models and templates', href: '/console/hub' },
        { name: 'Deployments', note: 'One control plane', href: '/console/deployments' },
      ]
        .map(
          (m) =>
            `<a class="card" href="${esc(m.href)}"><h3>${esc(m.name)}</h3><p>${esc(m.note)}</p></a>`
        )
