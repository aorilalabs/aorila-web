...
const PAGES = {
  ...
  pods: { title: 'Pods', badge: 'Product', heading: 'On-demand GPUs across 31 regions.', lede: 'Sign in to open the console and deploy.' },
...
function renderMarketingPage(slug, { query, user } = {}) {
  ...
  if (slug === 'contact') {
    ...
    extra = `<div class="cta-row"><a class="cta primary" href="https://atraly.com">Open Atraly</a><a class="cta ghost" href="https://api.aorila.com">Developer</a></div>`;
    below =
      '<section class="section undivided"><h2>Models on atraly.com</h2><div class="offer-grid">' +
      ATRALY_MODELS.map(
        (m) =>
          `<a class="offer-card" href="${esc(m.href)}"><div class="offer-gpu">${esc(m.name)}</div><div class="offer-meta">${esc(m.note)}</div><div class="offer-meta">atraly.com</div></a>`
      ).join('') +
      '</div></section>';
  } else if (LIVE_SLUGS.has(slug)) {
    extra = `<div class="cta-row"><a class="cta primary" href="/console">Open console</a><a class="cta ghost" href="/signup?next=/console">Create account</a></div>`;
    ...
    extra = `<div class="cta-row"><a class="cta primary" href="/console">Console</a><a class="cta ghost" href="/docs">Docs</a></div>`;
  }
