...
const LIVE_SLUGS = new Set(['pods', 'serverless', 'clusters', 'hub', 'deployments', 'models', 'pricing']);

const PAGES = {
  models: { title: 'AI Models', badge: 'Product', heading: 'AI Models.', lede: 'Pick a model stack and a live GPU. Deploy starts a pod after you sign in.' },
  pods: { title: 'Pods', badge: 'Product', heading: 'On-demand GPUs across 31 regions.', lede: 'Live Aorila price. Press Deploy to start. Sign in first if you are not logged in.' },
  serverless: { title: 'Serverless', badge: 'Product', heading: 'Serverless GPU endpoints.', lede: 'Live capacity. Deploy an endpoint after you sign in.' },
  clusters: { title: 'Clusters', badge: 'Product', heading: 'Multi-node GPU clusters.', lede: 'Live multi-GPU offers. Deploy starts the job after you sign in.' },
  hub: { title: 'Hub', badge: 'Product', heading: 'Models and templates.', lede: 'Deploy open-source stacks on live Aorila GPUs.' },
  deployments: { title: 'Deployments / Models', badge: 'Product', heading: 'One control plane.', lede: 'Your hardware or ours. Live SKUs below.' },
...
function renderMarketingPage(slug, { query, user } = {}) {
  ...
  if (slug === 'contact') {
    extra = `<div class="cta-row"><a class="cta primary" href="/commercial">Commercial</a><a class="cta ghost" href="mailto:api@aorila.com">api@aorila.com</a></div>`;
    ...
    extra = `<div class="cta-row"><a class="cta primary" href="/login?next=${encodeURIComponent('/' + slug)}">Sign in to deploy</a><a class="cta ghost" href="/signup?next=${encodeURIComponent('/' + slug)}">Create account</a></div>`;
