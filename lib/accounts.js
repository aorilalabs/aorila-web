...
function createAccountStore(options = {}) {
  ...
  const empty = () => ({ users: [], machines: [], credits: [], bookings: [] });
  function load() {
  ...
  function signup({ email, password, name, company, plan }) {
    const key = String(email || '').trim().toLowerCase();
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(key)) { const err = new Error('Use a valid email.'); err.status = 400; throw err; }
    ...
    const user = { id: id('usr'), email: key, name: String(name || key.split('@')[0]).trim(), company: String(company || '').trim(), plan: plan === 'commercial' ? 'commercial' : 'on-demand', password: hashPassword(password), createdAt: nowIso() };
  ...
  function createMachine(userId, payload) {
    const state = load();
    const machine = { id: id('pod'), userId, sku: String(payload.sku || ''), name: String(payload.name || payload.sku || 'Aorila GPU'), usdPerHour: Number(payload.usdPerHour || 0), region: String(payload.region || ''), tier: String(payload.tier || 'on-demand'), status: payload.status || 'starting', remoteId: payload.remoteId || null, createdAt: nowIso(), updatedAt: nowIso(), message: payload.message || '' };
  ...
  return { signup, login, issueSession, readSession, publicUser, findUserById, balance, machinesFor, bookingsFor, addCredits, createMachine, updateMachine, createBooking };
}
